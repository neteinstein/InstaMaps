...
- **Test a link**: `feature:settings`'s Settings screen has a "Test a link" field (visible once
  the Gemini API key is saved) that runs a pasted Instagram/TikTok link through the exact same
  pipeline a real OS share triggers, without leaving Settings. `SettingsRoute` exposes this as an
  `onTestLink(url)` callback rather than calling into `feature:share` directly - feature modules
  never depend on each other directly - so `MainActivity` (which already depends on both) supplies
  it, feeding the URL into the same hoisted `sharedText` Compose state `ShareRoute` already
  consumes for real shares. `ShareRoute` also takes a `sharedTextRequestId: Long` nonce alongside
  `sharedText`, bumped on every submission (real share or test link alike): Compose's
  `LaunchedEffect(sharedText)` wouldn't otherwise restart when the exact same URL is submitted
  twice in a row, since the state value is structurally unchanged
- **In-app updates**: `core:update` checks GitHub Releases (`GET
  ...
  Once the system Package Installer actually finishes installing that APK, a manifest-declared
  `UpdateApkCleanupReceiver` (`core:update`) listening for `ACTION_MY_PACKAGE_REPLACED` deletes it
  from `context.cacheDir/updates/` via `ClearDownloadedUpdateUseCase`, so the file doesn't sit
  there wasting space - deleting it any earlier (e.g. right after firing the install `Intent`)
  would risk the Package Installer still reading it through its `FileProvider` URI
- **Localization**: English (default `values/`) and Portuguese (`values-pt/`) - every module with
  user-facing strings (`app`, `feature:settings`, `feature:share`, `feature:permissions`,
  `feature:instagramauth`, `feature:history`) ships both. Per-app language switching is Android
  13+'s `android:localeConfig` manifest attribute pointing at `app/src/main/res/xml/locales_config.xml`
  (listing `en`/`pt`) - since `targetSdk` is already 36 (>= 33), this alone makes "App language"
  show up under the system Settings app entry and applies the chosen locale with zero runtime
  code. `feature:settings` additionally surfaces a "Language" section - the first thing on the
  Settings screen, ahead of even the API key field, since it's a device-wide preference rather
  than something tied to getting InstaMaps working - with a button deep-linking straight into
  `Settings.ACTION_APP_LOCALE_SETTINGS`; hidden entirely below API 33, where no such system screen
  exists (mirrors `core:permissions`'s `POST_NOTIFICATIONS` SDK-gating convention)
...
button is actually pressed, rather than being part of the onboarding gate above. The downloaded
APK cleans itself up automatically (`UpdateApkCleanupReceiver`) once installed - nothing to
configure or remember to clear.

Two more `feature:settings` additions need no setup either: "Test a link" (visible once the
Gemini API key is saved) runs a pasted Instagram/TikTok link through the full pipeline without
needing a real share - handy for verifying a key/permission change actually works end to end -
and "Language" (at the very top of the screen) deep-links into the system's per-app language
picker on API 33+ so a device set to Portuguese can run InstaMaps in English, or vice versa,
independent of the device's own language.
...
- `UpdateApkCleanupReceiver` follows the same "kept thin" rule: it's a plain `BroadcastReceiver`
  that only checks the broadcast action and delegates to `ClearDownloadedUpdateUseCase` behind
  `goAsync()`, with no logic of its own worth a Robolectric-backed test. `ClearDownloadedUpdateUseCase`
  and `GitHubUpdateRepository.clearDownloadedUpdate()`'s directory-deletion logic are exercised the
  normal way instead - the use case via `UpdateUseCasesTest`'s fake repository (mirroring
  `DownloadAppUpdateUseCaseTest`), same as every other Context-dependent repository method in this
  codebase.
